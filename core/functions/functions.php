...
function delete_preferred_reviewers($user)
{
    $user = sanitize($user);
...
function add_preferred($reviewer, $reviewee, $user)
{
    ...
    if ($reviewer == $reviewee) {
        ...
        $query = mysql_query("	SELECT *
			FROM preferred_poll
			WHERE 
				Reviewer = (SELECT ID FROM user WHERE UPPER(Username) = UPPER('$reviewer'))	AND
				Reviewee = (SELECT ID FROM user WHERE UPPER(Username) = UPPER('$reviewee'))	AND
				User = (SELECT ID FROM user WHERE UPPER(Username) = UPPER('$user'))	AND
				Batch = $batch
		");
        if (!$query || mysql_num_rows($query) < 0) {
            echo mysql_error();
...
{
    return (isset($_SESSION['user_name'])) ? $_SESSION['user_name'] : 'Not logged in';
}
...
//        get_text('Reviews_given_to_preferred_reviewee') . ": <b>$preferred_reviewees</b><br />";
    ...
    <p>
        1: <?php echo get_answer_name(1); ?><br/>
        2: <?php echo get_answer_name(2); ?><br/>
        3: <?php echo get_answer_name(3); ?><br/>
        4: <?php echo get_answer_name(4); ?><br/>
        5: <?php echo get_answer_name(5); ?>
    </p>
    <table class="questions">
        ...
        <tr style="margin-top: 25px;">
            ...
            <th><?php echo get_text('Your_score'); ?></th>
        </tr>
        </thead>
        <tbody>
        <?php
...
{ // Selecteer de 5 beste polls voor een reviewee
    $reviewee = (int)$reviewee;
...
{ // Selecteer de 5 beste polls voor een reviewer
    $reviewer = (int)$reviewer;
