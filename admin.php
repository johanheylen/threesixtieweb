            ...
            <div class="topContent">
                ...
                if (get_running1_batch_id()) {
                    ...
                    if ($users) {
                        ...
                        ?>
                        <p style="width: 100%;">
                            <b><?php echo $number; ?></b> <?php echo get_text('Users_have_not_filled_in_own_poll'); ?>.
                        </p>
                    ...
                    if ($users) {
                        ...
                        <p style="width: 100%;">
                            <b><?php echo $number; ?></b> <?php echo get_text('Users_have_not_filled_in_other_poll'); ?>
                            .</p>
                        ...
                        if (isset($_POST['reminder_2'])) {
                            ...
                            <?php
                            foreach ($users as $user) {
                                send_reminder_phase2($user['Username'], $user['Email']);
                            }
                    ...
                    foreach ($parameters as $parameter) {
                        ...
                        <tr>
                            <form action="" method="post">
                                ...
                                <td><input type="text" value="<?php echo $parameter['Value']; ?>" size="1"
                                           id="value<?php echo $parameter['ID']; ?>"/></td>
                                <script>
                                    parameter<?php echo $parameter['ID']; ?> = document.getElementById("value<?php echo $parameter['ID']; ?>");
                                </script>
                                <td><input type="submit" value="<?php echo get_text('Edit'); ?>"
                                           onclick="edit_parameter(<?php echo $parameter['ID']; ?>,parameter<?php echo $parameter['ID']; ?>.value);"/>
                                </td>
                            </form>
                        </tr>
                    <?php
                            ...
                            foreach ($categories as $category) {
                                ...
                                foreach ($questions as $key => $question) {
                                    if ($question['Category'] == $category['ID']) {
                                        ?>
                                        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post"
                                              class="questions_list" name="save_question">
                                            <table>
                                                <tbody>
                                                <tr>
                                                    <?php
                                                    if ($question['ID'] == $_POST['question_id']) {
                                                        ?>
                                                        <td style="width: 100%;"><textarea class="comment"
                                                                                           name="question_text"
                                                                                           id="question_text"><?php echo $question['Question']; ?></textarea>
                                                        </td>
                                                    <?php
                                                    } else {
                                                        ?>
                                                    ...
                                                    <td>
                                                        ...
                                                        if ($question['ID'] == $_POST['question_id']) {
                                                            ...
                                                            <input type="submit" name="save"
                                                                   value="<?php echo get_text('Save'); ?>"/>
                                                        <?php
                                                        } else {
                                                            ?>
                                                            <input type="submit" name="edit"
                                                                   value="<?php echo get_text('Edit'); ?>"/>
                                                            <!--</td>
															<td>
...
?>
    <aside class="topSidebar">
        ...
        <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            <label for="firstname"><?php echo get_text('Firstname'); ?>: </label><br/><input type="text" id="firstname"
                                                                                             name="firstname" required/><br/>
            <label for="lastname"><?php echo get_text('Lastname'); ?>: </label><br/><input type="text" id="lastname"
                                                                                           name="lastname"
                                                                                           required/><br/>
            ...
            <label for="email"><?php echo get_text('Email'); ?>: </label><br/><input type="text" id="email" name="email"
                                                                                     required/><br/>
            <label for="job_title"><?php echo get_text('Job_title'); ?>: </label><br/><input type="text" id="job_title"
                                                                                             name="job_title"/><br/>
    ...
    <aside class="middleSidebar">
        ...
        <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            <label for="user_department"><?php echo get_text('Department'); ?>: </label><br/><input type="text"
                                                                                                    id="user_department"
                                                                                                    name="department"
                                                                                                    required/><br/>
                ...
                <select id="manager" name="manager">
                    ...
                    if ($managers) {
                        foreach ($managers as $manager) {
                            ?>
                            <option
                                value="<?php echo $manager['ID']; ?>"><?php echo $manager['Lastname'] . ' ' . $manager['Firstname']; ?></option>
                        <?php
                        }
                    } else {
                        ?>
    ...
    foreach ($categories as $category) {
        ...
        foreach ($questions as $key => $question) {
            if ($question['Category'] == $category['ID']) {
                ?>
								<option value="<?php echo $question['ID']; ?>"><?php echo $key + 1;
